...
define('jira/autocomplete/jql-autocomplete', [
    ...
    var JQLAutoComplete = function(options) {
        ...
        that.setAutoSelectFirst = function (suggestionNodes) {
            if (!that.isAutoSelectFirstEnabled()){
                return;
            }
            var start = that.getReplacementStartIndex(result, "");
            var end = jQuery(that.field).selectionRange().end;
            var currentLexem = that.field.val().substring(start, end);
            if (currentLexem.length === 0 ||
                (suggestionNodes && suggestionNodes.length > 0 && Util.equalsIgnoreCase(currentLexem, suggestionNodes[0][1]))) {
                that.autoSelectFirst = false;
            } else {
                that.autoSelectFirst = true;
            }
        };
        ...
        that.renderSuggestions = function(suggestions, moreSuggestions, operatorSuggestions) {
            ...
            if (suggestions instanceof Array) {
                ...
                var escPrompt = jQuery('<span class="esc-prompt">')
                ...
                var syntaxHelpLink = jQuery('<a class="syntax-help" target="_jiraHelp"></a>')
                    ...
                    .text(formatter.I18n.getText('navigator.advanced.query.syntax'))
                    .append(escPrompt);
                var syntaxHelpContainer = jQuery('<div class="syntax-help-container">')
                    .append(syntaxHelpLink);
        ...
        that.needsSpaceInCompletion = function () {
            ...
            return that.isAutoSelectFirstEnabled() && !(result.isInsideList() || result.getNeedsOpenParen());
        };
        ...
        that._afterCompleteField = function (e) {
            ...
            if (that.needsSpaceInCompletion()) {            //We need to check the same conditions here as for spaces
        ...
        that.isAutoSelectFirstEnabled = function() {
...
define('jira/jql/jql-parse-token', [
    ...
    return function() {
        return /** @lends JQLAutoComplete.Token.prototype */ {
            ...
            setInListParens: function() {
                this.inListParens++;
            },

            setOutOfListParens: function() {
