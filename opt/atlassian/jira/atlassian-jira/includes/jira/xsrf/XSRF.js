 ...
 * the page.
 */
define('jira/xsrf', ['jira/util/formatter', //    'aui',
'jquery'], function(
    formatter,
    //    AJS,
    $) {
    ...
    var tokenQueryParam = function(token) {
        return formatter.format('{0}={1}', PARAM_TOKEN_NAME, token);
    };
    ...
    var replaceTokenInMeta = function(oldToken, newToken) {
        var metaTokenSelector = formatter.format('meta#{0}', META_TOKEN_ID);
    ...
    var replaceTokenInForms = function(oldToken, newToken) {
        $('form').each(function() {
            ...
            var formInputSelector = formatter.format('input[name="{0}"][value="{1}"]', INPUT_TOKEN_NAME, oldToken);
