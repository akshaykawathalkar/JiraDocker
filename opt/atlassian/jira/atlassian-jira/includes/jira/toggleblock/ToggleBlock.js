define('jira/toggleblock/toggle-block', [
    ...
    'jquery',
    'jira/libs/parse-uri'
], function(logger, Class, Events, Types, localStorage, $, parseUri) {
    ...
    return Class.extend({
        ...
        _updateTwixiBlockIdInStorage: function (blockId) {
            ...
            if (!this.isPermlink()) {
                ...
                var val = localStorage.getItem(this.options.storageCollectionName) || "";
                var blockLength = (',' + val + ',').indexOf(',' + blockId + ',') + 1;
                if (blockLength) {
        ...
        init: function (options) {
            ...
            if (!this.isUnique(uid)) {
                if (typeof console !== "undefined" && logger.warn) {
                    logger.warn(
                        "Your are trying to create a ToggleBlock with selector '" + this.options.triggerSelector + "'."
