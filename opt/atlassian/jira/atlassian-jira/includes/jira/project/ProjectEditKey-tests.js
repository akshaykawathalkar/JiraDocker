AJS.test.require(["jira.webresources:edit-project-details"], function() {
    ...
    function createField() {
        return '<div>' +
            '<input class="text" type="text" name="key" id="project-edit-key" value="' + ORIGINAL_KEY + '" maxlength="10">' +
            '<input type="hidden" id="edit-project-original-key" name="originalKey" value="' + ORIGINAL_KEY + '">' +
            '<input type="hidden" id="edit-project-key-edited" name="keyEdited" value="false">' +
            '</div>';
     }
    ...
    test("Change the key in the dialogue will pop the flag", function () {
        ...
        cancelFlag();
        pek.checkModified();
        equal(this.$field.find('#edit-project-key-edited').val(), "false",
            "now the project-key is not marked as modified after flag-reset-value");
        equal(this.$field.find('#project-edit-key').val(), ORIGINAL_KEY,
            "the project key has the original value after flag-reset-value");
        ok(!flagShowing(), "No flag when the project-key value is the original one");
    });
});
