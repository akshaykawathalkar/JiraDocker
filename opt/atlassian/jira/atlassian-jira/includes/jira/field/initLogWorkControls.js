define('jira/field/init-log-work-controls', [
    'jira/util/events',
    ...
    function applyLogworkControls(context) {
        ...
        jQuery(
            '#log-work-adjust-estimate-new-value, #log-work-adjust-estimate-manual-value',
            context
        ).attr('disabled','disabled');

        jQuery('#log-work-adjust-estimate-' + jQuery(
            'input[name=worklog_adjustEstimate]:checked,input[name=adjustEstimate]:checked',
            context
        ).val() + '-value', context).removeAttr('disabled');
