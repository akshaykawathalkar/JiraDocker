        ...
        validateName: function(name) {
            if (name == this.initialName && this.$nameElement.parent().find(".error").size()) {
                ...
                if (name.length > this.maxNameLength) {
                    this.showInlineError(this.$nameElement, this.initialName, formatter.I18n.getText("admin.errors.project.name.too.long", this.maxNameLength));
                    return;
                }
                for (var i = 0, ii = this.projectNames.length; i < ii; i++) {
                    if (name.toUpperCase() == this.projectNames[i]) {
                        this.showInlineError(this.$nameElement, this.initialName, formatter.I18n.getText("admin.errors.project.with.that.name.already.exists"));
                        return;
                    }
                }
            }
