define('jira/field/favourite-picker', ['jira/util/formatter', 'jquery'], function(formatter, $) {
    var FavouritePicker = function(options) {
        ...
        var theUrl = contextPath + "/rest/api/1.0/" + urlType + "/" + entityId + "/favourite";
        var originalOrder = [];

        var initOriginalOrderArray = function() {
            $("#" + options["tableId"]).find("[id^=fav_div_]").each(function(i, domElem) {
                originalOrder[originalOrder.length] = $(domElem).attr("rel");
            });
        };

        var addUndoAction = function() {
        ...
        var invalidateFiltersList = function() {
            if (relatedDropdown){
