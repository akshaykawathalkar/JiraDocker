...
JIRA.Share.EditSharesController.prototype =
{
    ...
    addDisplay : function(shareDisplay)
    {
        ...
        if (shareDisplay.singleton)
        {
            ...
            if (this.shares.length !== 0)
            {
                var msg = JIRA.Share.i18n.getMessage("common.sharing.remove.shares.public");
                if (shareDisplay == "loggedin") {
                    msg = JIRA.Share.i18n.getMessage("common.sharing.remove.shares.authenticated");
                }
 ...
 */
JIRA.Share.AuthenticatedUserShare = function()
{
    this.type = "loggedin";
    this.singleton = true;
};

JIRA.Share.AuthenticatedUserShare.prototype =
{
    /**
     * Return the Display that needs to be rendered when the user configures a new GLOBAL "ShareType" using
     * the GUI.
    ...
    {
        var newPermission = new JIRA.Share.SharePermission(this.type, null, null);
        var inner = JIRA.Share.i18n.getMessage("share_authenticated_display");
        return new JIRA.Share.Display(inner, this.getDisplayDescriptionFromUI(), this.singleton, newPermission);
    },
    ...
    getDisplayDescriptionFromUI : function()
    {
        return JIRA.Share.i18n.getMessage("share_authenticated_description");
    },
...
JIRA.Share.registerEditShareTypes = function(submitButtonId)
{
    ...
    editController.registerShareType(new JIRA.Share.AuthenticatedUserShare());
